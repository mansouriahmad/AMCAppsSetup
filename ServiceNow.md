...
- Under each entity, there will be DisplayFields child node where the fields related to the entity will be displayed on the softphone. Here Name corresponds to the string to be displayed on CRM app and Value represents the name of the property inside that entity.  

  For example, if you configure a an entry with name=Phone and value=primary_phone, if an entity is a match and it has property called primary_phone as 8045551234, inside CRM "Phone 8045551234" will be shown.
...
##### 2.7.6.1.1 autoPopulateScreenPop:
...
```
Phone|csm_consumer|business_phone;
```
...
| Key | Value |
|--------|-------|
|ServiceNow Table to Create Entity| \<Name to Display for Entity\> \| \<Image to Show in Quick Create Section\> |
...
### 2.8.2. AutoCloseInteractionRecord:
...
**Note**: To link activities with various entities in your ServiceNow instance, add records for each entity type in the following format:

| key | value |
|-----|-------|
|AMC_RelatedTo_**\<entity APIName\>**|Entity's property name in the interaction table|


For example, add one records with:

**key**=AMC_RelatedTo_csm_consumer, **value**: consumer.
...
**URL:** This URL determines which version of AMC's ServiceNow app is loaded.
